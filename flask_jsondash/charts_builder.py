...
def view(c_id):
    ...
    allmodules = adapter.readmodules()
    str1=",height':'400px','order':0,'refresh': true,'refreshInterval': 5000,'row': 1,'width':"
    str1+="'600px'}" if layout_type == 'freeform' else "'col-4'}" 
    for category,modules in allmodules.items():
        for module in modules:
            # module['module']=re.sub(r"'cachedData':\ {^\}*}","'cachedData':{}",module['module'])
            # module['module']=eval(module['module'][:-1]+",'guid':'"+module['guid']+"'"+",'name':'"+module['name']+"'"+str1)
            module['module']=eval(module['module'].replace('\n','').replace('\r',''))
            module['module']['guid']=module['guid']
            module['module']['name']=module['name']
            module['module']['height']='400px'
            module['module']['width']='600px' if layout_type == 'freeform' else 'col-4'
            module['module']['refresh']=True
            module['module']['row']=1
            module['module']['refreshInterval']=5000
            module['module']['cachedData']=None
            module['module'].pop('cachedData')
    kwargs = dict(
...
@charts.route('/charts/<c_id>/addmodule', methods=['POST'])
def addmodule(c_id):
    view_url = url_for('jsondash.view', c_id=c_id)
    form_data = request.form
    module=eval(form_data.get('modules'))
    module['cachedData']=None
    quotedmodule=adapter.readmodules(guid=module['guid'])
    if len(quotedmodule)==0:
        flash('Invalid JSON config. "{}"'.format(e), 'error')
        return redirect(view_url)
    else:
        data=adapter.read(c_id=c_id)
