...
jsondash.getDynamicWidth = function(container, config) {
    if(isNaN(config.width)) {
        return Math.round(container.node().getBoundingClientRect().width);
    }
...
jsondash.getDiameter = function(container, config) {
    ...
    return d3.min([Math.round(width), config.height]);
};
...
jsondash.handlers.handleSigma = function(container, config) {
    ...
    if(container.select('.chart-container')[0][0].innerHTML!=null){
        container.select('.chart-container')[0][0].innerHTML='';
    }
...
jsondash.handlers.handleD3force = function(container, config) {
    // 'use strict';

    jsondash.getJSON(container, config, function(error, data){

    var div = container.select('.chart-container');
    var _width = isNaN(config.width) ? jsondash.getDynamicWidth(container, config) : config.width;
    if(div[0][0].innerHTML==''){
        div= container
        .select('.chart-container').append('div')
        .classed(jsondash.util.getCSSClasses(config))
        .style('position', 'relative')
        .style('margin-top', 40 + 'px')
        .style('display', 'none')
        .style('width', _width + 'px');
        //默認顯示全屏，縮小到canvas顯示，配合d3force.js中的函數實現
        //實在沒辦法，需要用這種方法
        t=setTimeout(function(){ senceupdate(div,_width-2,config.height-80);},1000);

        Graph=ForceGraph3D()(div[0][0]).backgroundColor(config.bgcolor) ;
        div.select('canvas')[0][0].setAttribute('width',_width-2 + 'px');
    div.select('canvas')[0][0].setAttribute('height',config.height-80 + 'px');
    }

        Graph.graphData(data);

        // //边的样式，
        Graph.linkAutoColorBy('id')
        .linkWidth('size')
        .linkOpacity(0.4)
...
jsondash.handlers.handleForce2d = function(container, config) {
    // 'use strict';
    var div = container.select('.chart-container');
    if(container.select('.chart-container').select('svg')[0][0]!=null){
        container.select('.chart-container').select('svg')[0][0].innerHTML='';
        var svg = container
            .select('.chart-container')
            .select('svg'); 
    }else{
    ...
    jsondash.getJSON(container, config, function(error, data){
        ...
        var w = _width - jsondash.config.WIDGET_MARGIN_X;


        var linkForce = document
          .forceLink()
          .id(function (link) { return link.id })
          .strength(0.5)

        var simulation = container
          .forceSimulation()
          // .force('link', linkForce)
          .force('charge', container.forceManyBody().strength(-120))
          .force('center', container.forceCenter(width / 2, height / 2))
...
jsondash.handlers.handleFlameGraph = function(container, config) {
    jsondash.getJSON(container, config, function(_, data){
        ...
        d3.select("body").select(".d3-flame-graph-tip").remove();
        jsondash.api.runCallbacks(container, config);
        jsondash.unload(container);
    });
};
...
jsondash.handlers.handleVegaLite = function(container, config) {
    ...
    jsondash.getJSON(container, config, function(error, vlspec){
        ...
        vg.embed(selector, embedSpec, function(error, result) {
            ...
            if(d3.select(selector)[0][0].innerHTML==''){
...
jsondash.handlers.handleEcharts = function(container, config) {
    var _width = isNaN(config.width) ? jsondash.getDynamicWidth(container, config) : config.width;
    jsondash.unload(container);
    ...
    jsondash.getJSON(container, config, function(error,data){
        ...
        if(div[0][0].innerHTML==''){
            ...
            .style('width', _width + 'px')
            .style('margin-top', 40 + 'px')
            .style('height', config.height-60 + 'px');
            myChart=echarts.init(div[0][0],theme);
            myChart.on('click',function(params){
            param=[{'name':'category','value':params['name']}];
            transform_chart(param,jsondash.widgets['widgets']);
        });
        }else{
            div= container.select('.chart-container')
            .select('div');
            myChart=echarts.init(div[0][0],theme);
        }


        if(config.type=='map'){//地图模式
            addScript(`/static/js/vendor/map/${data.mapType}.js`);
            datas={};
            datas={
                tooltip: {
                  show: false
            ...
            for (var i = 0; i < data.class_count; i++) {
            datas.series.push({ 'type': 'bar' })
            }
            data=datas;
        ...
        //   var dataLen = data.series[0].data.length;
        //   // 取消之前高亮的图形
...
jsondash.handlers.handleDataTable = function(container, config) {
    ...
    jsondash.getJSON(container, config, function(error, res) {
        ...
        if(container.select('.chart-container').select('table')[0][0]!=null){
            container.select('.chart-container')[0][0].innerHTML='';
        }
        container
            .select('.chart-container')
...
jsondash.handlers.handleNumbersGroup = function(container, config) {
    ...
    jsondash.getJSON(container, config, function(error, data){
        ...
        if(container.select('.chart-container').select('table')[0][0]!=null){
            container.select('.chart-container').select('table').select('tr')[0][0].innerHTML='';
            var table=container
            .select('.chart-container')
            .select('table')
            .select('tr')
...
jsondash.handlers.handleSingleNum = function(container, config) {
    ...
    var _width = isNaN(config.width) ? jsondash.getDynamicWidth(container, config) : config.width;
    jsondash.getJSON(container, config, function(error, res){
        var data = res.number ? res.number : res.number;
    var div = container.select('.chart-container');
    if(div[0][0].innerHTML==''){
        div= container
        .select('.chart-container').append('div')
        ...
        var num=div.classed({singlenum: true})
            .classed(jsondash.util.getCSSClasses(config))
            .text(data);
        data = String(data);
...
jsondash.handlers.handleIframe = function(container, config) {
    ...
    if(div[0][0].innerHTML==''){
        div= container
        .select('.chart-container').append('iframe')
    }else{
        div= container.select('.chart-container')
        .select('iframe');
    }
...
jsondash.handlers.handleCustom = function(container, config) {
    ...
    $.get(config.dataSource, function(html){
        div.classed({'custom-container': true})
            .classed(jsondash.util.getCSSClasses(config))
            .html(html);
        // Look for callbacks potentially registered for third party code.
        jsondash.api.runCallbacks(container, config);
...
jsondash.handlers.handleVenn = function(container, config) {
    ...
    if(cont[0][0].innerHTML==''){
        ...
        cont= container.select('.chart-container')
        .select('div');
    }
        ...
        cont.select('svg')
            .attr('width', _width - jsondash.config.WIDGET_MARGIN_X)
...
jsondash.handlers.handlePlotly = function(container, config) {
    ...
    var _width = isNaN(config.width) ? jsondash.getDynamicWidth(container, config) : config.width;
    // debugger;
    var cont = container.select('.chart-container');
    if(cont[0][0].innerHTML==''){
        container
        .select('.chart-container')
        .append('div')
    ...
    jsondash.getJSON(container, config, function(error, data){
        ...
        if(config.override) {
            Plotly.newPlot(id, data.data, data.layout || {}, data.options || {});
        } else {
